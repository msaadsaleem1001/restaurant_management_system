...
if  (isset($_POST['Delete_report']))
{
	...
	$serial_del = $_POST['Delete_report'];
	// print_r($Item_del);
	$sql = "DELETE FROM `reported-issues` WHERE `reported-issues`.`Sr` = '$serial_del';";
...
function Alert_display_for_del_success()
			...
			<div class='alert alert-success alert-dismissible fade show' role='alert'>
						<strong>Success!</strong> Selected report has been deleted successfully.
...
function Alert_display_for_del_error()
			...
			<div class='alert alert-danger alert-dismissible fade show' role='alert'>
						<strong>Error!</strong> Selected report has not been deleted due to internal server Error.
						<button type='button' onclick=\"this.parentElement.style.display='none';\" class='btn-close' data-bs-dismiss='alert' aria-label='Close'></button>
			</div>";
			}
	if(1)
			...
			<table class='table table-bordered text-center table-hover table-responsive' id='myTable'>
  				<thead class='bg-dark text-white'>
    				<tr>
  					...
  					<tbody>";
					  	require 'partials/db_connect.php';
						$sql = 'SELECT * FROM `reported-issues`';
    					...
    					echo "<tr>
      						...
      						<td style='padding-top:20px;'>".$row['Description-report']."</td>
							<td style='padding-top:20px;'>".$row['Adress-mail']."</td>
							<td style='padding-top:20px;'>".$row['Date']."</td>
							<td><button class='delete_report btn btn-lg btn-danger'><i class='fa fa-trash'></i></button></td>
							<td style='display:none;'>".$row['Sr']."</td>
						</tr>";
						}
						$conn->close();
...
<form id="delete_report_form" action="admin_issues.php" method="POST" style="display:none;">
			<input type="number" id="Delete_report" name="Delete_report">
</form>
...
<script>
	...
	deletes = document.getElementsByClassName('delete_report');
    Array.from(deletes).forEach((element) => {
      element.addEventListener("click", (e) => {
		tr = e.target.parentNode.parentNode.parentNode;
		Delete_report.value = Number(tr.getElementsByTagName("td")[5].innerText);
